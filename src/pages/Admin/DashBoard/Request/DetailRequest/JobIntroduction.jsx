...
export default function JobIntroduction({ data, jobId }) {
  const salaryRanges = formatSalaryRangeToVND(
    data?.salaryRange || "Thương lượng"
  );
  ...
  return (
    <div className="p-6 bg-white w-[39rem] h-fit">
      ...
      <div className="flex items-center justify-between gap-3">
        <button className="bg-lime-500 text-white font-semibold py-2 w-[50rem] px-6 rounded-full hover:bg-lime-600 transition" onClick={() => { 
          navigate("/admin?page=request")
        }}>
          Quay lại duyệt tin tuyển dụng
        </button>
      </div>
    </div>
  );
