...
const EditJob = () => {
    ...
    const districtOptions = selectedProvince
        ? (areaData.find((item) => item.province === selectedProvince.value)?.districts || [])
            .map((district) => ({
                label: district,
                value: district,
            }))
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setNotification({ type: 'error', message: 'Không thể tải dữ liệu. Vui lòng thử lại sau.' });
            }
        };
    ...
    const handleSave = async () => {
        try {
            if (!jobData || !jobId) {
                Modal.error({
                    title: 'Lỗi',
                    content: 'Không thể cập nhật tin tuyển dụng. Thiếu thông tin cần thiết.'
                });
                return;
            }
    ...
    // Custom input styles
    const inputFieldClass = "w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors bg-white";
    const selectFieldClass = "w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors bg-white";
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="form-group">
                                    ...
                                    <DatePicker
                                        ...
                                        value={formData.workDateIn ? dayjs(formData.workDateIn) : null}
                                    />
                                </div>
                            </div>
                        </div>
