  ...
  return (
    <div className="relative rounded-lg overflow-hidden my-1">
      ...
      <div
        ...
        style={{
          width: `${DELETE_AREA_WIDTH}px`,
          transform: `translateX(${DELETE_AREA_WIDTH}px)`,
        }}
        onClick={(e) => {
          e.stopPropagation();
          onDeleteNews(job.id);
      ...
      >
          ...
          <div className="flex flex-col flex-grow gap-y-1">
            <div className="flex flex-col sm:flex-row sm:justify-between font-bold text-sm group-hover:text-[#1b8e0c] transition-colors duration-300">
              ...
              <div className="flex flex-row items-center justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[0.7rem] font-medium ${statusData.class}`}
                >
                  {statusData.icon}
                  {job?.status || ""}
                </span>
        ...
        <div className="flex flex-row justify-end items-center gap-2">
          <button
            className={`px-3 py-1 text-white rounded-full text-xs whitespace-nowrap transition-colors ${
              job?.status === "APPROVED"
            ...
            title={
              job?.status !== "APPROVED"
                ? "Chỉ có thể sửa tin đã được duyệt"
                : "Sửa tin tuyển dụng"
            }
          >
            Sửa tin
