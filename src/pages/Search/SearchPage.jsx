...
export default function SearchPage() {
    ...
    const filterParams = useMemo(() => {
        ...
        if (filters.profession?.length) params.profession = filters.profession.join(',');
        if (filters.area) params.area = filters.area;
        if (filters.experience && filters.experience !== 'Tất cả') params.experience = filters.experience;
        if (filters.jobLevel && filters.jobLevel !== 'Tất cả') params.jobLevel = filters.jobLevel;
        if (filters.salaryMin != null) params.salaryMin = filters.salaryMin;
        if (filters.salaryMax != null) params.salaryMax = filters.salaryMax;
        if (filters.workType && filters.workType !== 'Tất cả') params.workType = filters.workType;
        if (filters.companyIndustry) params.companyIndustry = filters.companyIndustry;
        if (filters.sortBy) params.sortBy = filters.sortBy;
        if (filters.order) params.order = filters.order;
        return params;
    }, [filters]);

    const { data: jobs = [], loading, error } = useCustomFetch(
